...
def default_unit_conversion_factor():
    ...
    In such way g/l will be converted as *1000 mg/l.
...
def default_map_header():
    """
    Default map headers are a couple of variable names that can used in HGC.io, which include LocationID, Datetime, SampleID,
    Feature, Unit, and Value. To run HGC.io, we have a define a dictionary to map map headers to the names mentioned above. 
    Here we generate a default dictionary with the mapping of headers in the original file (keys) equal headings in output df (values).
...
def read_file(file_path='', sheet_name=0, na_values=[], encoding='', delimiter=None, **kwargs):
    ...
    if (file_extension == 'xlsx') or (file_extension == 'xls'):
        try:
            ...
            df = pd.DataFrame()
            print('HGC has found a xlsx file but cannot read it.')
        ...
        try:
            ...
            df = pd.DataFrame()
            print('HGC has found a csv file but cannot read it.')
            # logger.error('WARNING file not read: ' + file_path)
    else:
        df = pd.DataFrame()
        print('HGC cannot find a file. Check the path.')
        # logger.error('WARNING file extension not recognized: ' + file_path)

    if df.empty:
        raise ValueError('df cannot be empty. Please make sure the file is placed in the same folder of this code and contains contents. \
            Or use a direct path, e.g. file_path = "C:\yourFolder\yourFile.xlsx" ')
