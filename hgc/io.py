...
def default_na_values():
    """
    Generate list of values that are recognizes as NaN.
...
def import_file(dataframe=None, file_path='', sheet_name=0, shape='stacked',
    ...
    The original file may contain either stacked or wide shaped data.
    ...
    slice_data : list/ slice,
        slice where to get the data (observations). See also Example.
    map_header : dict,
    ...
    map_units : dict, optional
        ...
        ==> Note 1: the values should be defined in SI units and case sensitive.
        ...
        PO4 mg/L P). The dictionary should then be defined as follows:
        {mg-N/L: mg/L N, mg-P/L: mg/L P} (see also Note 3).
    unit_conversion_factor : dict (optional)
        to convert units.
        Default is a dictionary of frequenty used waterquality units that is
    ...
    refer to certain rows an columns of the imported table.
    Example: "[9, slice(2, 5)]" refers to column 2 to 5 of row 9.
    In this function, slice does not work  with ":" symbol. Instead use "None".
    ...
    - with stacked shaped data, the first entry is kept.
    - with wide shape data, the feature/ unit that comes first in alphabetic order
    is kept. Note that they are ordered based on the original feature/ unit (so
    before mapping).
